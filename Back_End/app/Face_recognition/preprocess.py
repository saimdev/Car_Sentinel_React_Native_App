                    ...
                    if not os.path.exists(output_filename):
                        try:
                            ...
                            if nrof_faces > 0:
                                ...
                                if bb_temp[0] < 0 or bb_temp[1] < 0 or bb_temp[2] > img.shape[1] or bb_temp[3] > img.shape[0]:
                                    print('Invalid bounding box coordinates for "%s"' % image_path)
                                    text_file.write('%s\n' % (output_filename))
                                    continue

                                cropped_temp = img[bb_temp[1]:bb_temp[3], bb_temp[0]:bb_temp[2], :]
                                
                                if cropped_temp.shape[0] <= 0 or cropped_temp.shape[1] <= 0:
                                    print('Invalid cropped region for "%s"' % image_path)
                                    text_file.write('%s\n' % (output_filename))
                                    continue
                                ...
                                # else:
                                #     print('Invalid bounding box for "%s"' % image_path)
                            else:
                                print('Unable to align "%s"' % image_path)
                                text_file.write('%s\n' % (output_filename))
