...
# print(sys.argv[2])
# classifier_filename = './class/'+sys.argv[2]+'.pkl'
# npy='./npy'
# train_img="./train_img"
...
#                                     if HumanNames[best_class_indices[0]]==sys.argv[2]:
#                                         owner=True
...
#             out.write(frame)
#             print("WRITING")
#             with open("owner_status.txt", "a") as f:  # Open the file in append mode
#                 f.write(f"{str(owner)}\n")
#             print('OWNER STATUS WRITED')
#             key= cv2.waitKey(1)
#             if key== 113: # "q"
#                 break
#         out.release()
        ...
        if faceNum > 0:
                ...
                for i in range(faceNum):
                    ...
                    try:
                        ...
                        best_class_indices = np.argmax(predictions, axis=1)
                        best_class_probabilities = predictions[np.arange(len(best_class_indices)), best_class_indices]
                        print("CLASSES:", best_class_probabilities)
                        if best_class_probabilities>0.60:
                            cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)    #boxing face
                            for H_i in HumanNames:
                                if HumanNames[best_class_indices[0]] == H_i:
                        ...
                        print("Saved processed image with recognition results:", output_image_path)
                        with open("owner_status.txt", "a") as f:  # Open the file in append mode
                            f.write(f"{str(owner)}\n")
                        print('OWNER STATUS WRITED')
                    except:   
