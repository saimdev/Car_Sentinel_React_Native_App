...
const FinalProductScreen = ({navigation}) => {
  ...
  useEffect(() => {
    // AsyncStorage.clear('dataCollect');
    const retrieveSettings = async () => {
      try {
        const userDataJSON = await AsyncStorage.getItem('userData');
        const dataCollection = await AsyncStorage.getItem('dataCollect');
        if (userDataJSON) {
    ...
    const checkDataCollection = () => {
        if (data===null){
            navigation.navigate('CarDataCollection');
        }
    }
  ...
  const handlePredictButtonPress = async () => {
    ...
    const apiUrl = 'http://'+api+':8000/pervious_motion_detection'
    console.log(apiUrl);
    if (!selectedVideo) {
      return;
    }
  ...
  return (
    <ImageBackground
      source={require('../assets/motionbg.png')}
    ...
    paddingHorizontal: 20,}} disabled={true}>
      ...
      {loading && !videoLink && <Text style={{marginVertical:10, color:"white", fontSize:15, textAlign:"center", }}>Motion Detection Started..... It will Take sometime</Text>}
